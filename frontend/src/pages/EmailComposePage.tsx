...
export function EmailComposePage() {
  ...
  async function submit() {
    ...
    if (!window.confirm(`${sendMode === 'now' ? 'Send now' : 'Schedule'} ${summary.recipients_ready} recipient emails? ${summary.invalid_emails} invalid addresses will be skipped.`)) return
    setBusy(true); setError('')
    try {
      await api.createCampaign({ ...selection, name, email_template_id: templateId, sender_name: sender, reply_to: replyTo || null, subject, body, attach_certificate: attach, send_mode: sendMode, scheduled_local: sendMode === 'schedule' ? localTime : undefined, timezone, confirmed: true })
  ...
  return <><div className="admin-page-heading"><div><span className="eyebrow">/ EMAIL</span><h1>COMPOSE<span>.</span></h1><p>Create a personalized certificate campaign.</p></div><Link className="button button-outline" to="/admin/email/templates">MANAGE TEMPLATES →</Link></div>
    ...
    <section className="admin-info-card email-form"><h2>DELIVERY</h2><label>When<select value={sendMode} onChange={e => setSendMode(e.target.value as 'now' | 'schedule')}><option value="now">Send now</option><option value="schedule">Schedule</option></select></label>{sendMode === 'schedule' && <><label>Date and time<input type="datetime-local" value={localTime} onChange={e => setLocalTime(e.target.value)} /></label><label>Timezone<input value={timezone} onChange={e => setTimezone(e.target.value)} /></label></>}<div className="certificate-actions"><button disabled={busy || !summary?.recipients_ready || (sendMode === 'schedule' && !localTime)} onClick={() => void submit()}>{sendMode === 'now' ? 'SEND NOW' : 'SCHEDULE CAMPAIGN'}</button></div><small>Review the recipient summary first. Final confirmation appears before delivery begins. Large campaigns continue in scheduled batches.</small></section>
  </>
}
