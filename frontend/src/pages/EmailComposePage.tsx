...
export function EmailComposePage() {
  ...
  const [selection, setSelection] = useState<RecipientSelection>({ selection: 'all', send_to: 'unsent' })
  ...
  async function showSummary() { try { setSummary(await api.recipientSummary({ ...selection, name, attach_certificate: attach })); setError('') } catch (e) { setError(e instanceof Error ? e.message : 'Summary failed') } }
  ...
  return <><div className="admin-page-heading"><div><span className="eyebrow">/ EMAIL</span><h1>COMPOSE<span>.</span></h1><p>Create a personalized certificate campaign.</p></div><Link className="button button-outline" to="/admin/email/templates">MANAGE TEMPLATES →</Link></div>
    ...
    <section className="admin-info-card email-form"><h2>MESSAGE</h2><label>Campaign name<input value={name} onChange={e => { setName(e.target.value); setSummary(null) }} /></label><label>Email template<select value={templateId ?? ''} onChange={e => chooseTemplate(e.target.value ? Number(e.target.value) : null)}><option value="">Custom message</option>{templates.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}</select></label><label>Sender display name<input value={sender} onChange={e => setSender(e.target.value)} /></label><label>Reply-to address<input type="email" value={replyTo} onChange={e => setReplyTo(e.target.value)} /></label><label>Subject<input value={subject} onChange={e => { setSubject(e.target.value); setPreview(null) }} /></label><label>Body<textarea rows={9} value={body} onChange={e => { setBody(e.target.value); setPreview(null) }} /></label><label className="email-check"><input type="checkbox" checked={attach} onChange={e => { setAttach(e.target.checked); setSummary(null) }} /> Generate and attach each certificate on demand</label><div className="certificate-actions"><button onClick={() => void showPreview()}>PREVIEW WITH SAMPLE DATA</button></div>{preview && <div className="email-preview"><strong>{preview.subject}</strong><pre>{preview.body}</pre></div>}</section>
